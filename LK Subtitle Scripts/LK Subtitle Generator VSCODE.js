{
    ...
    // LK Subtitle Generator.jsx | LK字幕生成工具

    /* My Code Style

        1. Search "TODO" and "FIXME" to see what had not accomplished.
        2. Search "TEST" to see which options were on developing.
        3. Change "version_env" value to switch between develop-environment and released-environment.

        Updated at 2018-05-24 230846 
    */
    ...
    function LK_SubtitleGenerator() {
        // ======================== Init Environment ========================

        var scriptName = 'LK Subtitle Generator'; // 脚本标题
        var version_env = 'DEVELOP'; // 切换到“开发者版本” // TEST
        ...
        var edt = null; // 文本输入框
        var gen_mode_btn_1 = null; // 控制生成器工作模式
        var gen_mode_btn_2 = null;
        ...
        function createUI() {
            ...
            */
            ...
            // 生成器工作模式
            // FIXME
            gen_mode_btn_1 = panel.add('radiobutton', [280, 5, 380, 23], 'full_automatic', {});
            gen_mode_btn_2 = panel.add('radiobutton', [390, 5, 490, 23], 'semi_automatic', {});
            gen_mode_btn_1.value = true;
        ...
        var file = DataCenter('commonFilePath'); // 文件对象
        var id = 0; // 列表编号
        ...
        function presetStaticValues() {
            ...
            if (version_env == 'RELEASE') {
                ...
                if (!SCRIPT_FILE.exists) {
                    ...
                    SCRIPT_FILE = new File(
                        file.path + '/LK Subtitle Scripts/LK字幕导入工具.jsxbin'
                    );
                    ...
                    if (!SCRIPT_FILE.exists) {
                        ...
                        SCRIPT_FILE = File.openDialog(
                            "Look for 'LK Subtitle Import.jsxbin'",
                            [
                                'Adobe Script File Binary:*.jsxbin',
                                'Adobe Script File:*.jsx',
                                'All Files:*.*'
                            ],
                            false
                        );
        ...
        function addListeners() {
            ...
            gen_mode_btn_1.onClick = onGenMode1Click;
            gen_mode_btn_2.onClick = onGenMode2Click;
        }
        ...
        function getFilePath(scriptName) {
            var path = DataCenter('commonFilePath');
            lkLog("getFilePath() - this file's path is " + path);
         ...
         * @return {Object} app.project.item(index)
         */
        ...
        /**
         * 获取当前视频所在的时间点的时间值
         * @param {String} style 需要返回什么样式的时间值，可选“FRAME”（“0:00:32:02”）
         * 或“SECONDS”（“32.15649812315”）
         * @return {String} timecode
         */
        function getCurrentTime(style) {
            var item = getItem(0);
            var timecode = '';
            switch (style) {
                case 'FRAME':
                    timecode = item.time;
                    timecode = timeToCurrentFormat(timecode, item.frameRate);
                    break;
                case 'SECONDS':
                    timecode = item.time;
                    break;
        ...
        function onBtn3Click() {
            ...
            if (m == n) {
                // 如果`m = n`，则说明删除的是最后一项，所以只需要将id值-1即可
                id -= 1;
            } else {
                for (id = m * 1; m <= id < n; id++) {
                    // 这里之所以定义`id=m*1`，是因为m的形式是“03”（字符串类型），因此我们要对它转成数字“3”
                    // 个位数需要进行补零操作：01, 02, 03...
                    if (id < 10) {
        ...
        var generate_mode = 'full_automatic';
        var layer_a = null;
        var layer_b = null;
        var layer_c = null;
        function onBtn4Click() {
            ...
            */
            ...
            if (gen_mode_btn_1.value) {
                generate_mode = 'full_automatic';
            } else {
                generate_mode = 'semi_automatic';
            }
            ...
            if (generate_mode == 'full_automatic') {
                if (id > 0) {
                    for (var i = 0; i < id; i++) {
                        ...
                        if (app.project.item(INDEX).layers.byName('temp_layer_1') != null) {
                            app.project
                                .item(INDEX)
                                .layers.byName('temp_layer_1')
                                .remove();
                            app.project
                                .item(INDEX)
                                .layers.byName('temp_layer_2')
                                .remove();
                        }
                    }
                } else {
                ...
                // 'semi_automatic'
                changeStatusIni('reset');
                lkLog('changeStatusIni() - status reset ok', 'p');
                ...
                if (generate_counter == 0) {
                    ...
                    */
                    ...
                    lkLog('onBtn4Click() - generate_counter = ' + generate_counter + '/' + id, 'p');
                    ...
                    lkLog('parsingSubtitleContents() - ok', 'p');
                    ...
                    generate_counter = -1; // 因为后面会进行一次 ++ 操作，所以这里设为 -1，这样重置以后最终变为 0
                    lkLog('onBtn4Click() - Generate work done\n\n', 'p');
                }
        ...
        function getTempLayers(layerName, format) {
            // FIXME use getItem instead of master
            var master = app.project.item(INDEX);
            var layer = master.layers.byName(layerName);

            if (layer == null) {
                // Create layers
                layer = master.layers.addText('temp');
                layer.name = layerName;
                layer = setFontProperties(layer, layerName, format);
            }
        ...
        function createTextLayer() {
            var master = app.project.item(INDEX);
            var layer = master.layers.addText('main');
            layer.name = 'sub' + generate_counter;
            layer = setFontProperties(layer, layer.name, 'layer_a');
            return layer;
        }

        function setFontProperties(layer, str, format) {
            // 字体格式处理
            ...
            /*
                ...
                - Web:
                    ...
                    - [【AE脚本】关于AE文字图层如何设置字体类型的问题研究](http://likianta.gitee.io/2018/0325-132316/)
            */
            ...
            switch (format) {
                case 'layer_a':
                    // 上大字（示例“奶油”）
                    s.font = 'FZLTZHK--GBK1-0'; // 方正兰亭中黑_GBK
                    s.fontSize = 90;
                    break;
                case 'layer_b':
                    // 下小字数字（示例“123”）
                    s.font = 'ArialMT'; // Arial
                    s.fontSize = 72;
                    break;
                case 'layer_c':
                    // 下小字汉字（示例“茶匙”）
                    s.font = 'FZLTXHJW--GB1-0'; // 方正兰亭纤黑简体 代码
                    s.fontSize = 72;
                    break;
                default:
                    break;
        ...
        function getSubtitleContents(index) {
            var subtitle = listbox.items[index - 1];
            var contents = new Array();
            contents[0] = subtitle.subItems[0].text; // get start time
            if (index == 1 && contents[0].search(/-/g) != -1) {
                contents[0] = '0:00:00:00';
            }
            contents[1] = subtitle.subItems[1].text; // get end time
            ...
            contents[2] = subtitle.subItems[2].text; // get subtitle content
            return contents;
        }
        ...
        function parsingSubtitleContents(layer_a, layer_b, layer_c, contents) {
            ...
            var receipt = 0;
            var str = contents[2];
            lkLog('parsingSubtitleContents() - pure subtitle string is ' + str, 'p');

            // Separate str into 3 parts: text_a, text_b, text_c
            receipt = regexMe(str);

            function regexMe(str) {
                var receipt = 0;
                /*
                    回执：receipt = 0, 1, 2, 3, 4
                    0: 初始检测
                    1: 有b，有c
                    2: 有b，无c（不存在此种情况）
                    3: 无b，有c
                ...
                */
                /*
                    Samples: 
                    1. 奶油 1茶匙（常见，上大字和下小字之间有一个空格分隔）
                    2. 奶油1茶匙（常见，原始字幕没有分隔符）
                    3. 奶油\N1茶匙（常见，上大字和下小字之间由“\N”分隔）
                    4. 奶油 约1茶匙
                    5. 奶油 1-5茶匙
                    6. 奶油 少许（不常见，但属于正常用法）
                    7. 奶油（常见，这个是只有上大字的情况）
                    8. 3茶匙奶油（不常见）
                    9. 33茶匙奶油（跟上个一样。这个是为了测试多个数字的情况）
                    10. 33茶匙 奶油（不常见，好像目前还没有这种形式的原始字幕）
                    11. 奶油\N少许
                */
                var p, q, r;
                // 0是上大字的开始位置
                ...
                // q是下小字数字的开始位置
                // r是下小字汉字的开始位置
                // str.length是下小字汉字的结束位置

                p = str.search(/[\d\s\\½⅓⅔¼¾⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞约]/gm);
                q = str.search(/[\d½⅓⅔¼¾⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞]+/gm);
                r = str.search(/[^N\d\s½⅓⅔¼¾⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞%]+$/gm);

                switch (p) {
                    case -1:
                        fillinTheLayer(layer_a, str + ' ', 'layer_a');
                        receipt = 4;
                        break;
                    case 0:
                        fillinTheLayer(layer_a, str + ' ', 'layer_a');
                        ...
                        break;
                    default:
                        fillinTheLayer(layer_a, str.slice(0, p) + ' ', 'layer_a');
                        switch (q) {
                            case -1:
                                fillinTheLayer(layer_c, str.slice(r), 'layer_c');
                                receipt = 3;
                                break;
                            default:
                                switch (r) {
                                    case -1:
                                        fillinTheLayer(layer_b, str.slice(q), 'layer_b');
                                        receipt = 2;
                                        break;
                                    default:
                                        fillinTheLayer(layer_b, str.slice(q, r), 'layer_b');
                                        fillinTheLayer(layer_c, str.slice(r), 'layer_c');
                                        receipt = 1;
                                        break;
                                }
            ...
            // 注意：fillinTheLayer()不能放到父函数外面，否则它将无法连入contents数组，会导致layer_a解析失败
            function fillinTheLayer(layer, str, format) {
                lkLog('fillinTheLayer() - ' + layer.name + ' text = ' + str, 'p');
                switch (format) {
                    case 'layer_a':
                        layer.inPoint = contents[0];
                        layer.outPoint = contents[1];
                    default:
                        setFontProperties(layer, str, format);
                        break;
                }
            }
        ...
        function launchGenius() {
            ...
            if (version_env == 'DEVELOP') {
                ...
                script = getFilePath('/LK Subtitle Scripts/LK AE Genius.exe');
            }

            // Run
            //alert("按键精灵程序即将启动，点击确认以继续……", "Tip");
            script.execute();
        }
        ...
        function changeStatusIni(receipt) {
            ...
            /*
                ...
                layer_b=true|false
                layer_c=true|false
                status=stop|resume|pause|exit|error|reset
            */
            ...
            */
            ...
            switch (receipt) {
                ...
                case 4:
                    // 无b无c
                    // PS：如果下小字都没有，只有上大字，则按键精灵只需去点一下生成按钮
                    report[2] = 'layer_b=false';
                    report[3] = 'layer_c=false';
                    report[4] = 'status=resume_d'; // directly click btn4(dx, dy)
                    break;
                case 'error':
                    // 发生未知错误，立刻退出
            ...
            if (version_env == 'DEVELOP') {
                ...
                file = getFilePath('/LK Subtitle Scripts/resources/current_status.ini');
            }
            file.open('w');
            ...
            if (regex.test(content)) {
                ...
                for (var i = 0; ; i++) {
                    ...
                    if (s != null || s != undefined) {
                        ...
                        alert(
                            'Script stopped because of countless matches. \nPlease check the type of source file which you selected.',
                            'Warning'
                        );
                        break;
                    } else {
        ...
        function TimecodeFromAegToAE(i) {
            ...
            function AccurateTime(time) {
                ...
                if (timecode != '0:00:00.000') {
                    t =
                        timecode.slice(0, 1) * 3600 +
                        timecode.slice(2, 4) * 60 +
                        timecode.slice(5, 7) * 1 +
                        timecode.slice(-2) / 100; // = 0 hour // = 00 minute // = 06 seconds // = 810ms/1000 = 0.81s
                    // = 6.81s
        ...
        function onGenMode1Click() {
            lkLog('onGenMode1Click() - full_automatic', 'p');
            gen_mode_btn_1.value = true;
            generate_mode = 'full_automatic';
        }
        ...
        function onGenMode2Click() {
            lkLog('onGenMode2Click() - semi_automatic', 'p');
            gen_mode_btn_2.value = true;
            generate_mode = 'semi_automatic';
        }
    }

    LK_SubtitleGenerator();
}
