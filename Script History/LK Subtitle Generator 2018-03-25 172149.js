            ...
            */
            ...
            if (generate_mode == 'full_automatic') {
                ...
                function parsingSubtitleContents(layer_a, layer_b, layer_c, contents) {
                    ...
                    if (p != -1) {
                        // Sample 1, 2, text a
                        fillInTheLayers(layer_a, str.slice(0, p + 1), 'text_a'); // '奶油 '，注意末尾有一个空格
                        ...
                        if (p != -1) {
                            // Sample 1, '1茶匙'
                            fillInTheLayers(layer_b, str.slice(p, q), 'text_b'); // '1'
                            fillInTheLayers(layer_c, str.slice(q), 'text_c'); // '茶匙'
                            // Done
                            receipt = 1;
                        } else if (q != -1) {
                            // Sample 2, '少许'
                            fillInTheLayers(layer_c, str, 'text_c'); // '少许'
                            // Done
                            receipt = 3;
                        } else {
                            // Unknown error
                        ...
                        fillInTheLayers(layer_a, str, 'text_a');
                        // Done
                        receipt = 4;
                    }

                    function fillInTheLayers(layer, text, format) {
                        lkLog('fillInTheLayers() - layer = ' + layer.name + ' - text = ' + text, 'p');
                        switch (format) {
                            case 'text_a':
                                lkLog('fillInTheLayers() - contents length = ' + contents.length, 'p');
                ...
                function changeStatusIni(receipt) {
                    ...
                    */
                    ...
                    // 阅读（分析）回执
                    lkLog('changeStatusIni() - receipt = ' + receipt, 'p');
                    switch (receipt) {
                        case 0:
                            // 所有工作已完成，正常退出运行
