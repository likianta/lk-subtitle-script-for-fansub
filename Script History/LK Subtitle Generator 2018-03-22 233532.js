{
    ...
    // LK字幕生成工具
    //
    // 发布注意：发布前请将`var version_env = 'DEVELOP'`注释掉，并启用`var version_env = 'RELEASE'`
    ...
    function LK_SubtitleGenerator(thisObj)
    {
        ...
        function createUI() {
            ...
            slider.value = 0;
            slider_txt = panel.add('statictext', [350,28,400,40], 'offset: 0');
        ...
        slider.onChange = function () {
            if (slider.value != 0) {
                ...
                slider_txt.text = 'offset: ' + Math.round(slider.value);

                // 将该值保存到`config.txt`文件中。并在启动脚本时导入
                // ...
            }
        }
        ...
        function onBtn4Click() {
            ...
            */
            ...
            var txt_layer = 'layer_a'; // 定义一个区间变量，用于记录文字图层对象
            ...
            if (generate_mode = 'full_automatic') {
                if (id > 0) {
                    for (i = 0; i < id; i++) {
                        ...
                        if (app.project.item(INDEX).layers.byName('tmp_layer_1') != null) {
                            tmp_layer_1.remove();
                            tmp_layer_2.remove();
                        }
                    }
                ...
                if (generate_counter == 0) {
                     ...
                     * 5. 精灵进入等待状态
                     */
                    layer_c = createTmpLayers(layer_c, 'type_c');
                    layer_b = createTmpLayers(layer_b, 'type_b');
                    
                    // Launch LK Anjian Genius.exe
                    launchGenius();
                } else {
                    /**
                     * 工作步骤
                ...
                function createTmpLayers(layerName, format) {
                ...
                function launchGenius() {
                    // Get LK Anjian Genius.exe
                    var script = getFilePath('/LK Anjian Genius.exe');
                    // Run
                    script.execute();
                }
            ...
            function multiplyTextFormats(event) {
            	
            	
            	// Split the source text into 2 parts: “上大字(txt_a)”、“下小字(txt_b)”
            	// Sample 1: '奶油 1茶匙'
            	// Sample 2: '奶油 少许'
            	// 查询该字幕中是否存在“空格”或“换行符”
            	var s = content.search(/\s/g); // s = 2
            	if (s != -1) {
            		// If space char exists
            		var txt_a = content.slice(0, s); // '奶油'
            		var txt_b = content.slice(s); // ' 1茶匙'

            		txt_layer.property("Source Text").setValue(new TextDocument(txt_a));

            		// 查询下小字非数字部分的字符串的起始位置
            		s = txt_b.search(/[^\d]/g); // s = 1
            		if (s > 0) {
            			var txt_b_1 = txt_b.slice(0, s); // = ' 1'
            			var txt_b_2 = txt_b.slice(s); // = '茶匙'

            			tmp_layer_1.property("Source Text").setValue(new TextDocument(txt_b_1)); // ' 1'
            			callAnjianScript(1);
            			tmp_layer_2.property("Source Text").setValue(new TextDocument(txt_b_2)); // '茶匙'
            			callAnjianScript(2);
            		} else {
            			// 如果 s == 0，那就说明下小字中没有数字，比如这种情况：“奶油 少许”
            			tmp_layer_2.property("Source Text").setValue(new TextDocument(txt_b)); // '少许'
            			callAnjianScript(3);
            		}
            	} else {
            		txt_layer.property("Source Text").setValue(new TextDocument(content));
            	}
