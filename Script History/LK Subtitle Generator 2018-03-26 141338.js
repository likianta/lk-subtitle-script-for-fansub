{
    ...
    /*
        ...
        1. Search "TODO", "FIXME" or "UPDATEME" to see what had not accompished yet.
    ...
    */
    ...
    function LK_SubtitleGenerator(thisObj)
    {
        ...
        // TODO
        function CommonDataCenter(request) {
            switch (request) {
                case 'getCurrentTime':
            ...
            */
            ...
            if (generate_mode == 'full_automatic') {
                ...
                function parsingSubtitleContents(layer_a, layer_b, layer_c, contents) {
                    ...
                    p = str.search(/\s/g); // 看看是否存在空格，如果文字都是紧挨着的，那就不解析了（都当做上大字处理了）
                    if (p != -1) {
                        // Sample 1, 2, text a
                        lkLog('parsingSubtitleContents() - ready to fillinTheLayer, target str is ' + str.slice(0, p+1), 'p');
                        fillinTheLayer(layer_a, str.slice(0, p+1), 'layer_a'); // '奶油 '，注意末尾有一个空格

                        str = str.slice(p + 1); // '1茶匙' or '少许'
                        //var regex_b = RegExp('\\d.*\\d', 'g'); // 用于匹配“1茶匙”中的“1”、“5-10克”中的“5-10”
                        //var regex_c = RegExp('[^\\d-\\s].+$', 'g'); // 用于匹配“1茶匙”中的“茶匙”、“5-10 克”中的“克”
                        p = str.search(/\d/g); // 用于匹配“1茶匙”中的“1”
                        q = str.search(/[^\d-]+$/g); // 用于匹配“1茶匙”中的“茶匙”、“5-10 克”中的“克”
                        if (p != -1) {
                            // Sample 1, '1茶匙'
                            fillinTheLayer(layer_b, str.slice(p, q), 'layer_b'); // '1'
                            fillinTheLayer(layer_c, str.slice(q), 'layer_c'); // '茶匙'
                            // Done
                            receipt = 1;
                        } else if (q != -1) {
                            // Sample 2, '少许'
                            fillinTheLayer(layer_c, str, 'layer_c'); // '少许'
                            // Done
                            receipt = 3;
                        } else {
                            // Unknown error
