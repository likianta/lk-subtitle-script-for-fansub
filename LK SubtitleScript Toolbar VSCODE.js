{
    ...
    */
    ...
    // ---------------- Register the Top-Leval Variables ----------------
    var commonFilePath = function() {
        // Get relative path
        var file = new File($.fileName);
        return file.path+'/LK Subtitle Scripts/';
    };

    var commonConfig = function(commonFilePath) {
        var file = new File(commonFilePath+'/LK Subtitle Scripts/resources/config.ini');
        file.open('r');
        var str = file.read();
        file.close();
    ...
    function IndexDataCenter(key) {
        switch (key) {
            case 'commonFilePath':
                return commonFilePath ? commonFilePath : (commonFilePath = initData(key));
    ...
    function LK_SubtitleScriptToolbar(thisObj) {
        ...
        function checkFiles() {
            if (version_env == 'RELEASE') {
                ...
                SCRIPT_Examiner = new File(
                    file.path + '/LK Subtitle Scripts/LK Subtitle Examiner.jsx'
                );
                SCRIPT_Generator = new File(
                    file.path + '/LK Subtitle Scripts/LK Subtitle Generator.jsx'
                );
                ...
                switch (event) {
                    case 'Check Examiner':
                        if (!SCRIPT_Examiner.exists) {
                            SCRIPT_Examiner = new File(
                                file.path + '/LK Subtitle Scripts/LK后期检查工具.jsxbin'
                            );
                            if (!SCRIPT_Examiner.exists) {
                    ...
                    case 'Check Generator':
                        if (!SCRIPT_Generator.exists) {
                            SCRIPT_Generator = new File(
                                file.path + '/LK Subtitle Scripts/LK字幕生成工具.jsxbin'
                            );
                            if (!SCRIPT_Generator.exists) {
                                alert('Script file missed. You can browse it manually.', 'Prompt');

                                SCRIPT_Generator = File.openDialog(
                                    "Look for 'LK Subtitle Generator.jsxbin'",
                                    [
                    ...
                    case 'Check Help':
                        if (!SCRIPT_Help.exists) {
                            SCRIPT_Help = new File(
                                file.path + '/LK Subtitle Scripts/LK帮助.jsxbin'
                            );
                            if (!SCRIPT_Help.exists) {
        ...
        function createUI() {
            ...
            win =
                s instanceof Panel
                    ? s
                    : new Window('palette', scriptName, [100, 100, 370, 200], { resizeable: true });
        ...
        function launchGenerator() {
            ...
            require(commonFilePath+'LK Subtitle Generator.jsxbin');
        }
